...
- Runtime switch: the View → Theme menu radio updates `App::active_theme` and calls `theme::apply(ui.ctx(), …)`. Persistence on switch is deferred — the next launch picks up whatever the settings file says.
